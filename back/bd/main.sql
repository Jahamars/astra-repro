...
CREATE TABLE packages (
    id          SERIAL       PRIMARY KEY,
    name        VARCHAR(256) NOT NULL UNIQUE,  -- binary package name (apt install <name>)
    source_name VARCHAR(256),                  -- source package name (may differ)
    created_at  TIMESTAMPTZ  NOT NULL DEFAULT NOW()
);
...
CREATE TABLE package_versions (
    ...
    size_bytes            BIGINT,
    hash_declared         CHAR(64)     NOT NULL,  -- SHA256 from repo Packages index
    hash_download         CHAR(64),               -- SHA256 of actually downloaded .deb
    mirror_ok             BOOLEAN GENERATED ALWAYS AS (
                              CASE
                                  WHEN hash_download IS NULL THEN NULL
                                  ELSE hash_declared = hash_download
                              END
                          ) STORED,
    depends               TEXT,
    section               VARCHAR(64),
...
CREATE INDEX idx_pkg_versions_package_id    ON package_versions (package_id);
CREATE INDEX idx_pkg_versions_hash_declared ON package_versions (hash_declared);
CREATE INDEX idx_pkg_versions_mirror_ok     ON package_versions (mirror_ok)
    WHERE mirror_ok IS NOT NULL;
...
CREATE INDEX idx_runs_package_version_id ON verification_runs (package_version_id);
CREATE INDEX idx_runs_status             ON verification_runs (status);
CREATE INDEX idx_runs_finished_at        ON verification_runs (finished_at DESC NULLS LAST);
CREATE INDEX idx_runs_hashes_match       ON verification_runs (hashes_match)
    WHERE hashes_match IS NOT NULL;
CREATE INDEX idx_runs_snapshot_id        ON verification_runs (snapshot_id)
    WHERE snapshot_id IS NOT NULL;
-- for SKIP LOCKED queue polling
CREATE INDEX idx_runs_pending_queued     ON verification_runs (queued_at ASC)
...
-- Latest run per package version + mirror integrity status
...
LEFT JOIN verification_runs vr ON vr.package_version_id = pv.id
ORDER BY pv.id, vr.finished_at DESC NULLS LAST;

-- Aggregate stats by status (over latest run per version)
...
-- Critical and moderate diff issues with context
...
-- Full run history per package (for /packages/{name}/history)
...
-- Snapshot summary: how many runs per status per snapshot
...
VALUES ('local-astra-mirror', 'http://localhost/astra', '1.7_x86-64', 'main');

INSERT INTO build_environments (name, base_image)
VALUES ('astra-1.7-amd64-default', 'astralinux/alse:1.7');
