...
from services.comparer import ComparerService
...
async def create_run(
    ...
    pv = await conn.fetchrow(
        """
        SELECT pv.id, pv.hash_declared, pv.version, pv.arch,
    ...
    row = await conn.fetchrow(
        ...
        INSERT INTO verification_runs
            (package_version_id, environment_id, snapshot_id,
             hash_declared_at_run, triggered_by)
        VALUES ($1, $2, $3, $4, $5)
        RETURNING
            id, package_version_id, environment_id, snapshot_id,
            status, hash_declared_at_run, hash_rebuilt, hashes_match,
            source_date_epoch, build_path,
...
async def get_next_pending(
    ...
    row = await conn.fetchrow(
        """
        SELECT vr.id, vr.package_version_id, vr.environment_id,
               vr.snapshot_id, vr.status, vr.hash_declared_at_run,
               vr.hash_rebuilt, vr.hashes_match, vr.source_date_epoch,
               vr.build_path, vr.queued_at, vr.started_at,
               vr.finished_at, vr.build_duration_seconds,
               vr.failure_reason, vr.triggered_by,
...
async def start_run(
    ...
    """
    Builder сигнализирует что взял задачу.
    PENDING → BUILDING, проставляет started_at.
    """
    row = await conn.fetchrow(
        """
    ...
    if row is None:
        raise HTTPException(
            status_code=409,
            detail="Run not found or not in PENDING status",
        )
...
async def submit_result(
    ...
    svc = ComparerService(conn)
    try:
        result = await svc.process_builder_result(
            run_id=str(run_id),
            result=body,
            raw_diffs=None,
        )
...
async def submit_diffs(
    ...
    run = await conn.fetchrow(
        "SELECT id, status FROM verification_runs WHERE id = $1",
        str(run_id),
    )
    if run is None:
    ...
    from services.comparer import _classify_diff, DiffSeverity
    ...
    for diff in diffs:
        ...
        classified.append((
            ...
            diff.get("diffoscope_output"),
        ))

    async with conn.transaction():

        await conn.execute(
            "DELETE FROM diff_results WHERE run_id = $1", str(run_id)
        )
        ...
        new_status = (
            "NOT_REPRODUCIBLE_CRITICAL" if has_critical else "NOT_REPRODUCIBLE"
        )
        await conn.execute(
...
async def get_run(
    ...
    row = await conn.fetchrow(
        ...
        SELECT vr.id, vr.package_version_id, vr.environment_id,
               ...
               vr.build_path, vr.queued_at, vr.started_at,
               vr.finished_at, vr.build_duration_seconds,
               vr.build_log, vr.failure_reason, vr.triggered_by,
               p.name AS package_name, pv.version, pv.arch
        FROM verification_runs vr
        JOIN package_versions pv ON pv.id = vr.package_version_id
        JOIN packages p ON p.id = pv.package_id
    ...
    diffs = await conn.fetch(
        """
        SELECT id, run_id, file_path, section_name,
               cause, severity, description, created_at
        FROM diff_results
        WHERE run_id = $1
    ...
    return RunDetail(
        **dict(row),
        diffs=[DiffResultOut(**dict(d)) for d in diffs],
    )
